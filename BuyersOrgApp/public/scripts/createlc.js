const clickSign = function(){
    ...
    if(lc_id==""||secret==""||buyers_id==""||sellers_id==""||carrier_org_id==""||ammount==""||product_details_obj==""||expire_time==""){
...
const clickCreateLc = function(){
    ...
    if(lc_id==""||secret==""||buyers_id==""||sellers_id==""||carrier_org_id==""||ammount==""||product_details_obj==""||expire_time==""||pubkey==""||mysign==""){
        alert('input field should not be null');
        w.classList.add('addremove');
        return ;
    }
    ...
    let send_obj = {
        lc_id,
        buyers_public_key:pubkey,
        buyers_sign:mysign
    };
    fetch('http://localhost:3001/buyersverify',{
        method: 'POST',
        credentials : 'include',
        headers: {
            'Content-Type': 'application/json'
